...
"""
Sorunlu üst üste getirme ve basit fark bulma 
"""
...
def bit_xor(img1, img2, imgson, x1):
    ...
    for i in range(len(contours)):
        ...
        if area < 750:
            ...
            cv2.rectangle(imgson,(x+x1-5,y-5),(x+x1+w+5,y+h+5),(0,0,255),1)
...
iskelet1, kesisim1, points1 = skel(mask_alma(img1), 10)
...
while True:
    ...
    if bigArea < 10000:
        ...
        try:
            iskelet2, kesisim2, points2 = skel(roi,x1)
        except Exception:
        ...
        satir2.append(points2[i + 1])
        for a in range(i + 1, m - 1):
            x1, y1 = points2[a]
            x2, y2 = points2[a + 1]
            e = 40
            if abs(y2 - y1) < e:
                satir2.append(points2[a + 1])
            else:
                break
        if len(satir2) < 4:
        ...
        try:
            ...
            sonuc = bit_xor(mask_alma(warped), mask_alma(frame), frame,0)
