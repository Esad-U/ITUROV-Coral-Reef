...
"""
                            --- Coral Reef Main Code MultiThreading Deneme ---
    Thread kullanımı vardır. Gelen frame ler bir uçtan sıraya sokulur ve diğer uçtan da thread ler frame i alıp işler. 
İşlenen görüntüler sabit fps de ekrana yansıtılır. Tüm değişkenlere ve fonksiyonlara İngilizce isimler yazıldı. 
Ubuntu işletim sisteminde daha stabil çalışmaktadır.
...
def frame_processing(input_frame):
    coral_reef_after = Coral()
    coral_reef_after.get_image(input_frame)

    if not coral_reef_after.get_roi():
        output_frame = coral_reef_after.show_frame()
        return output_frame
    ...
    while True:
        if len(pending_task) < thread_num:
            ...
            if ret:
                ...
                task = pool.apply_async(frame_processing, (frame.copy(),))
                pending_task.append(task)

        while len(pending_task) > 6 and pending_task[0].ready():
            res = pending_task.popleft().get()

            nextFrame = time.time()
            fark = 0.03 - (nextFrame - lastFrame)
            if fark > 0:
                time.sleep(fark)
            waitFrame = time.time()
            fps = int(1 / (waitFrame - lastFrame))
            ...
            if fps > 25:
                ...
                print(fps, "!!!!!!!")
                counter += 1
            cv2.putText(res, str(fps), (840, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 0, 255), 2, cv2.LINE_AA)

            cv2.imshow("Video {0}x{1}".format(res.shape[1], res.shape[0]), res)
            lastFrame = time.time()

        if cv2.waitKey(1) == ord("q") or not ret:
            break
    end = time.time()
    print("Video su kadar surede oynatildi : ", int(end - start))
    print("Video ", counter, " kez fps dropu yedi")
