...
cap = cv2.VideoCapture("C:\\Users\\QP\\Desktop\\coralreeef_deneme\\QGroundControl 2020-11-27 13-13-56.mp4")

img1 = cv2.imread("2.1f.png")
copyimg1 = img1.copy()
...
while True:
    ...
    mask = mask_alma(frame)
    erode = cv2.erode(mask,np.ones((3,3),np.uint8),iterations=1)
    opening = cv2.morphologyEx(erode, cv2.MORPH_OPEN, kernel)
    dilation = cv2.dilate(opening,np.ones((3,3)),(3,3))
    opening = cv2.morphologyEx(dilation, cv2.MORPH_OPEN, kernel)
    closing = cv2.morphologyEx(opening, cv2.MORPH_CLOSE, kernel)
    cv2.imshow("Maskelenmis 2", closing)
