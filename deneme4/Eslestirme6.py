...
def sobelFoto(gImg,img,imgk,imgy,imgm,imgzero,renk):
    ...
    for i in range(len(contours)):
        ...
        if area > 50:
            (x, y, w, h) = cv2.boundingRect(contours[i])
            cv2.rectangle(imgzero, (x + 3, y), (x + w + 8, y + h), renk, -1)
    ...
    for i in range(len(contours)):
        ...
        if area > 30:
            (x, y, w, h) = cv2.boundingRect(contours[i])
            cv2.rectangle(imgzero, (x - 8, y), (x + w - 3, y + h), renk, -1)
    ...
    for i in range(len(contours)):
        ...
        if area > 40:
            (x, y, w, h) = cv2.boundingRect(contours[i])
            cv2.rectangle(imgzero, (x, y), (x + w, y + h + 5), renk, -1)
    ...
    for i in range(len(contours)):
        ...
        if area > 10:
            ...
            (x, y, w, h) = cv2.boundingRect(contours[i])
            cv2.rectangle(imgzero, (x, y - 5), (x + w, y + h), renk, -1)
...
points1,cikan1 = sobelFoto(img1,mask_alma_pembe(img1),img1k,img1y,img1m,img1zero,(0,0,255))
points2,cikan2 = sobelFoto(img2,mask_alma_pembe(img2),img2k,img2y,img2m,img2zero, (0,255,0))

_, cikan3 = sobelFoto(img1,mask_alma(img1),img1k,img1y,img1m,img1zero,(0,0,255))
_, cikan4 = sobelFoto(img2,mask_alma(img2),img2k,img2y,img2m,img2zero, (0,255,0))
...
warped2 = cv2.warpPerspective(cikan3, h, (width, height))
